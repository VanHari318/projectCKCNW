...
<div class="flex flex-col gap-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-white mb-2">Sửa bài tập</h1>
    ...
    <form method="POST" action="{{ route('teacher.assignments.update', $assignment) }}" class="space-y-4">
        ...
        <input type="text" name="title" class="w-full rounded form-input px-4 py-2" value="{{ $assignment->title }}" required>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input type="number" name="duration" class="w-full rounded form-input px-4 py-2" value="{{ $assignment->duration }}" min="1" required>
            <input type="datetime-local" name="due_date" class="w-full rounded form-input px-4 py-2" value="{{ $assignment->due_date?->format('Y-m-d\TH:i') }}" required>
        </div>

        <div class="bg-gray-900 p-4 rounded border border-gray-700">
            <p class="text-sm text-gray-300 mb-2">Chia sẻ bài tập sang khóa học khác</p>
