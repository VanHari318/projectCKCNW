...
@section('title', 'Nguồn Bài tập / Bài kiểm tra')

@section('content')
<div class="max-w-4xl mx-auto">
    <h1 class="text-2xl font-bold text-white mb-4">{{ __('Source Bài tập / Bài kiểm tra') }}</h1>
    ...
    <div class="bg-gray-800 p-4 rounded mb-4 flex flex-wrap items-center gap-4">
        <div>
            ...
            <select id="filterType" class="ml-2 rounded form-input px-3 py-1">
                <option value="assignment">{{ __('Bài tập') }}</option>
                <option value="quiz">{{ __('Bài kiểm tra') }}</option>
            </select>
        </div>
        <div>
    ...
    <div id="listContainer" class="space-y-4">
        ...
        {{-- === Bài kiểm tra chia theo khóa === --}}
        <div id="quizzesList" class="hidden">
            @php
...
<div id="repostModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded p-6 w-full max-w-xl">
        ...
        <form id="repostForm" method="POST" action="{{ route('teacher.assignments.store') }}">
            @csrf
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" id="repostSubmit" class="px-4 py-2 rounded btn-primary">Giao bài tập</button>
            </div>
        </form>
    </div>
</div>
