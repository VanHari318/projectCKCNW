...
@section('content')
    <div class="flex flex-col gap-8">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2">{{ __('Tham gia Lớp & Khóa học') }}</h1>
            <p class="text-gray-400">{{ __('Nhập mã lớp để tham gia hoặc chọn khóa học từ lớp đã tham gia') }}</p>
        </div>

        <!-- Join Classroom Section -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-gray-800 border border-purple-600 rounded-lg p-6">
                <h2 class="text-xl font-bold text-white mb-4">{{ __('Tham gia lớp mới') }}</h2>
                <form method="POST" action="{{ route('student.classrooms.join') }}" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm text-gray-300 mb-2">{{ __('Nhập mã lớp') }}</label>
            ...
            <div class="bg-gray-800 border border-purple-600 rounded-lg p-6">
                <h2 class="text-xl font-bold text-white mb-4">{{ __('Lớp học khác') }}</h2>
                    ...
                    <div class="space-y-2 max-h-64 overflow-y-auto">
                        @foreach($availableClassrooms as $classroom)
                            <div class="bg-gray-900 rounded p-3 border border-gray-700">
                                ...
                                <p class="text-sm text-gray-400">{{ __('Giáo viên') }}: {{ $classroom->teacher->name }}</p>
        ...
        @if($approvedClassrooms->count() > 0)
            <div class="bg-gray-800 border border-purple-600 rounded-lg p-6">
                <h2 class="text-xl font-bold text-white mb-4">{{ __('Lớp học của bạn') }} ({{ $approvedClassrooms->count() }})</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($approvedClassrooms as $classroom)
                        <div class="bg-gray-900 rounded p-4 border border-green-600">
                            <p class="text-white font-semibold">{{ $classroom->name }}</p>
                            <p class="text-sm text-gray-400 mb-3">{{ __('Giáo viên') }}: {{ $classroom->teacher->name }}</p>
        ...
        @if($pendingClassrooms->count() > 0)
            <div class="bg-gray-800 border border-yellow-600 rounded-lg p-6">
                <h2 class="text-xl font-bold text-white mb-4">{{ __('Chờ duyệt') }} ({{ $pendingClassrooms->count() }})</h2>
                <div class="space-y-3">
                    @foreach($pendingClassrooms as $classroom)
                        <div class="bg-gray-900 rounded p-4 border border-yellow-700">
                            <p class="text-white font-semibold">{{ $classroom->name }}</p>
                            <p class="text-sm text-gray-400 mb-3">{{ __('Giáo viên') }}: {{ $classroom->teacher->name }}</p>
                            <p class="text-yellow-400 text-sm mb-3">⏳ {{ __('Đang chờ giáo viên duyệt yêu cầu...') }}</p>
                            <form method="POST" action="{{ route('student.classrooms.leave', $classroom) }}">
                                @csrf
                                <button class="text-red-400 hover:text-red-300 text-sm" type="submit">
                                    {{ __('Hủy yêu cầu') }}
        ...
        @if($approvedClassrooms->count() > 0)
            <div class="bg-gray-800 border border-blue-600 rounded-lg p-6">
                <h2 class="text-xl font-bold text-white mb-4">{{ __('Khóa học có sẵn') }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($availableCourses as $course)
                            <div class="bg-gray-900 rounded p-4 border border-blue-700">
                                ...
                                <p class="text-xs text-gray-500 mb-3">{{ __('Lớp') }}: {{ $course->classroom->name }}</p>
