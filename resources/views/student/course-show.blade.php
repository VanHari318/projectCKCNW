...
@section('title', __('Chi tiết khóa học'))
...
<div class="flex flex-col gap-8">
    ...
    <!-- Course Info -->
    <div>
        <a href="{{ route('student.classrooms.show', $course->classroom) }}" class="text-purple-400 hover:text-purple-300 text-sm mb-4 inline-block">{{ __('← Quay lại lớp học') }}</a>
        <h1 class="text-3xl font-bold text-white mb-2">{{ $course->name }}</h1>
        <p class="text-gray-400">{{ __('Lớp:') }} {{ $course->classroom->name }}</p>
        <p class="text-gray-400">{{ __('Giáo viên:') }} {{ $course->classroom->teacher->name }}</p>
    </div>

    <!-- Course Stats -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-gray-800 p-4 rounded border border-gray-700">
            <p class="text-gray-400 text-sm">{{ __('Mô Tả') }}</p>
            <p class="text-white mt-2">{{ $course->description ?? __('Không có mô tả') }}</p>
        </div>
        <div class="bg-gray-800 p-4 rounded border border-gray-700">
            <p class="text-gray-400 text-sm">{{ __('Số Phòng Học') }}</p>
            <p class="text-2xl font-bold text-blue-400">{{ $rooms->count() }}</p>
        </div>
        <div class="bg-gray-800 p-4 rounded border border-gray-700">
            <p class="text-gray-400 text-sm">{{ __('Số Học Sinh Trong Khóa') }}</p>
    ...
    @if(!$isJoined)
        <div class="bg-yellow-900 border border-yellow-600 rounded-lg p-6">
            <p class="text-yellow-200 mb-4">{{ __('Bạn chưa tham gia khóa học này. Hãy tham gia để xem thêm thông tin.') }}</p>
    ...
    @else
        ...
        @if($rooms->count() > 0)
            <div class="bg-gray-800 border border-purple-600 rounded-lg p-6">
                <h2 class="text-2xl font-bold text-white mb-4">{{ __('Phòng Học') }}</h2>
        ...
        @else
            <div class="bg-gray-800 border border-yellow-600 rounded-lg p-6">
                <p class="text-gray-400">{{ __('Khóa học này chưa có phòng học nào được lên lịch.') }}</p>
            </div>
        @endif

        <div class="bg-gray-800 border border-green-600 rounded-lg p-6">
            <h2 class="text-2xl font-bold text-white mb-4">{{ __('Danh Sách Học Sinh Trong Khóa') }}</h2>
            ...
            @else
                <p class="text-gray-400">{{ __('Khóa học này chưa có học sinh nào tham gia.') }}</p>
            @endif
        </div>

        <!-- Assignments Section -->
        <div class="bg-gray-800 border border-blue-600 rounded-lg p-6">
            <h2 class="text-2xl font-bold text-white mb-4">{{ __('Bài Tập') }}</h2>
                            ...
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-sm {{ $isOverdue && !$grade ? 'text-red-400' : 'text-gray-400' }} mt-1">
                                        {{ __('Hạn:') }} {{ $assignment->due_date?->format('d/m/Y H:i') }}
                                        @if($isOverdue && !$grade)
                                            <span class="ml-2 text-xs bg-red-900 text-red-300 px-2 py-0.5 rounded">{{ __('Đã hết hạn') }}</span>
                                        @endif
                                    </p>
                                </div>
                                @if($grade)
                                    <span class="text-green-400 text-sm">{{ __('Điểm:') }} {{ $grade->score }}/10</span>
                                @endif
                            </div>
                            <div class="mt-3">
                                @if($grade)
                                ...
                                @elseif($isOverdue)
                                    <button type="button" onclick="alert('{{ __('Bài tập đã kết thúc! Bạn không thể làm bài này nữa.') }}')" class="text-red-400 hover:text-red-300 text-sm cursor-pointer">
                                        {{ __('Bài tập đã kết thúc ✕') }}
                                    </button>
                                @else
            ...
            @else
                <p class="text-gray-400">{{ __('Khóa học này chưa có bài tập.') }}</p>
            @endif
        </div>

        <!-- Bài kiểm tra -->
        <div class="bg-gray-800 border border-purple-600 rounded-lg p-6">
            <h2 class="text-2xl font-bold text-white mb-4">{{ __('Bài Kiểm Tra') }}</h2>
            @if($quizzes->count() > 0)
                <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="bg-gray-900 rounded p-4 border border-gray-700 hover:border-purple-500 transition">
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-sm {{ $isOverdue && !$grade ? 'text-red-400' : 'text-gray-400' }} mt-1">
                                        {{ __('Hạn:') }} {{ $quiz->due_date?->format('d/m/Y H:i') }}
                                        @if($isOverdue && !$grade)
                                            <span class="ml-2 text-xs bg-red-900 text-red-300 px-2 py-0.5 rounded">{{ __('Đã hết hạn') }}</span>
                                        @endif
                                    </p>
                            ...
                            <div class="mt-3">
                                ...
                                @elseif($isOverdue)
                                    <button type="button" onclick="alert('{{ __('Bài kiểm tra đã kết thúc! Bạn không thể làm bài này nữa.') }}')" class="text-red-400 hover:text-red-300 text-sm cursor-pointer">
                                        {{ __('Bài kiểm tra đã kết thúc ✕') }}
                                    </button>
                                @else
            ...
            @else
                <p class="text-gray-400">{{ __('Khóa học này chưa có bài kiểm tra.') }}</p>
            @endif
        </div>

        <!-- Leave Course Section -->
        <div class="flex justify-end">
            <form method="POST" action="{{ route('student.courses.leave', $course) }}" onsubmit="return confirm('{{ __('Bạn chắc chắn muốn rời khỏi khóa học này?') }}');">
                @csrf
                <button class="text-red-400 hover:text-red-300 text-sm" type="submit">
                    {{ __('Rời khỏi khóa học') }}
                </button>
            </form>
        </div>
    @endif
